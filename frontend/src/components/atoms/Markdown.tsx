  ...
  for (const match of text.matchAll(MENTION_PATTERN)) {
    ...
    nodes.push(
      <span
        ...
        className="font-semibold text-[var(--markdown-mention)]"
      >
        {mention}
      </span>,
    );
...
const MARKDOWN_CODE_COMPONENTS: Components = {
  pre: ({ node: _node, className, ...props }) => (
    <pre
      className={cn(
        "my-3 overflow-x-auto rounded-lg bg-[var(--code-block-bg)] p-3 text-xs leading-relaxed text-[var(--code-block-text)]",
        className,
      )}
      {...props}
    />
  ...
  code: (rawProps) => {
    ...
    if (isInline) {
      return (
        <code
          className={cn(
            "rounded bg-[var(--code-inline-bg)] px-1 py-0.5 font-mono text-[0.85em] text-[var(--code-inline-text)]",
            className,
          )}
          {...props}
        >
...
const MARKDOWN_TABLE_COMPONENTS: Components = {
  ...
  thead: ({ node: _node, className, ...props }) => (
    <thead
      className={cn("bg-[var(--markdown-table-header-bg)]", className)}
      {...props}
    />
  ),
  tbody: ({ node: _node, className, ...props }) => (
    <tbody
      className={cn(
        "divide-y divide-[var(--markdown-table-divide)]",
        className,
      )}
      {...props}
    />
  ...
  th: ({ node: _node, className, children, ...props }) => (
    <th
      className={cn(
        "border border-[var(--markdown-table-border)] px-3 py-2 text-left text-xs font-semibold",
        className,
      )}
      {...props}
    >
  ...
  td: ({ node: _node, className, children, ...props }) => (
    <td
      className={cn(
        "border border-[var(--markdown-table-border)] px-3 py-2 align-top",
        className,
      )}
      {...props}
    >
...
const MARKDOWN_COMPONENTS_BASIC: Components = {
  ...
  a: ({ node: _node, className, children, ...props }) => (
    <a
      className={cn(
        "font-medium text-[var(--markdown-link)] underline decoration-[var(--markdown-link-decoration)] underline-offset-2 transition-colors hover:text-[var(--markdown-link-hover)] hover:decoration-[var(--markdown-link-hover-decoration)]",
        className,
      )}
      target="_blank"
