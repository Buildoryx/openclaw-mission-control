    ...
    <Popover open={open} onOpenChange={handleOpenChange}>
      <PopoverTrigger asChild>
        <button
          ...
          disabled={disabled}
          className={cn(
            "inline-flex h-10 w-auto cursor-pointer items-center justify-between gap-2 rounded-md border border-[var(--input-border-strong)] bg-[var(--input-bg)] px-3 py-2 text-sm font-medium text-[var(--text)] shadow-sm transition-colors hover:bg-[var(--surface-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--input-focus-ring)] focus:border-transparent disabled:cursor-not-allowed disabled:opacity-50",
        ...
        >
          <span
            className={cn(
              ...
              showPlaceholder && "text-[var(--text-quiet)]",
            )}
          >
            {SelectedIcon ? (
              <SelectedIcon
          ...
          <ChevronDown
            className={cn(
              "h-4 w-4 shrink-0 text-[var(--text-quiet)] transition-transform",
              open && "rotate-180",
            )}
          />
        </button>
      ...
      <PopoverContent
        ...
        sideOffset={6}
        className={cn(
          "w-[var(--radix-popover-trigger-width)] min-w-[12rem] overflow-hidden rounded-md border border-[var(--popover-border)] bg-[var(--popover-bg)] p-0 text-[var(--popover-text)] shadow-lg",
          contentClassName,
        )}
      >
        <Command label={ariaLabel} className="w-full">
          ...
          <CommandList ref={listRef} className="max-h-64 p-1">
            <CommandEmpty className="px-3 py-6 text-center text-sm text-[var(--text-muted)]">
            ...
            {options.map((option) => {
              ...
              return (
                <CommandItem
                  ...
                  className={cn(
                    "flex items-center justify-between gap-2 rounded-lg px-4 py-3 text-sm text-[var(--text-muted)] transition-colors data-[selected=true]:bg-[var(--surface-strong)] data-[selected=true]:text-[var(--text)]",
                ...
                >
                  ...
                  {isSelected ? (
                    <Check className="h-4 w-4 text-[var(--text-quiet)]" />
                  ) : null}
                </CommandItem>
              );
            })}
